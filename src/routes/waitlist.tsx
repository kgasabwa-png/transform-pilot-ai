  ...
  head: () => ({
    meta: [
      { title: "Become a design partner - Receipts" },
      {
        name: "description",
        content:
          "Request a Receipts design-partner backtest and turn closed renewal calls into cited evidence, missed risks, and reviewed CS plays.",
      },
      { property: "og:title", content: "Become a design partner - Receipts" },
      {
        property: "og:description",
        content:
          "Run a concierge evidence backtest on closed renewals before connecting your stack. Every claim cited, every play human-reviewed.",
      },
    ],
  }),
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 grid lg:grid-cols-[1.1fr_0.9fr]">
        <section className="px-6 md:px-12 lg:px-16 py-16 md:py-24 max-w-2xl">
          <span className="eyebrow block mb-4">Evidence backtest - design partner preview</span>
          <h1 className="text-4xl md:text-5xl font-semibold tracking-tight leading-[1.05]">
            Show us closed renewals. We'll return the receipts your team missed.
          </h1>
          <p className="mt-5 text-muted-foreground text-base md:text-lg leading-relaxed max-w-xl">
            Send 5 to 15 anonymized closed renewals before you connect anything. We'll show which
            churns, expansions, and forecast misses were visible in the calls, with the exact
            customer moments behind each finding.
          </p>

          <ul className="mt-8 space-y-3 text-sm text-muted-foreground">
            {[
              "Your data, your overrides - every play awaits CSM signoff.",
              "Cited to the line. No uncited claims, no hallucinated quotes.",
              "You receive a missed-risk board, forecast memo, and CSM play review.",
            ].map((line) => (
            ...
            {status === "done" ? (
              <div className="rounded-2xl border border-border bg-background p-8">
                ...
                <p className="mt-3 text-sm text-muted-foreground leading-relaxed">
                  ...
                  session. In the meantime, the live evidence desk is open with no login.
                </p>
                <Link
                  to="/try"
                ...
                >
                  Open the evidence desk <ArrowUpRight className="size-4" />
                </Link>
              </div>
            ) : (
              <form
