          ...
          {messages.length === 0 && (
            <div className="rounded-xl border border-dashed border-border p-8 text-center">
              ...
              <p className="mt-3 text-[14px] text-foreground">
                Ask about meetings, decisions, customer themes, and follow-ups.
              </p>
              <p className="mt-1 text-[12.5px] text-muted-foreground">
                I only know the notes and context you've captured or connected.
              </p>
            </div>
          )}
            ...
            return (
              <div key={m.id}>
                <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">
                  {m.role === "user" ? "You" : "Nyvlo"}
                </div>
                <div className="mt-1.5 whitespace-pre-wrap text-[15px] leading-relaxed text-foreground/95">
                  {text}
                </div>
              </div>
            );
          ...
          <div className="mx-auto max-w-3xl">
            <div className="mb-2 flex flex-wrap gap-1.5">
              {prompts.map((p) => (
                <button
                  ...
                  onClick={() => send(p)}
                  disabled={loading}
                  className="rounded-full border border-border bg-card px-3 py-1 text-[11.5px] text-foreground/75 hover:bg-muted disabled:opacity-50"
                >
                  {p}
                </button>
              ))}
            ...
            >
              ...
              <button
                ...
                className="inline-flex h-7 w-7 items-center justify-center rounded-md bg-foreground text-background hover:opacity-90 disabled:opacity-50"
                aria-label="Send"
              >
